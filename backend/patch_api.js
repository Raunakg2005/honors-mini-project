const fs = require('fs'); let code = fs.readFileSync('./backend/src/index.ts', 'utf8'); code = code.replace(/app\.post\('\/api\/connect', \[\s\S\]*?\/\/ Keep existing jobs logic for now/, \pp.post('/api/connect', authMiddleware, async (req, res) => {
    try {
  ...
  // Keep existing jobs logic for now\); fs.writeFileSync('./backend/src/index.ts', code);
